...
resource "hcloud_server" "master-node" {
  ...
  user_data = templatefile("${path.module}/cloud-init.yaml", {
    ...
    worker_public_key = file("${path.module}/.mysecrets/id_rsa.hetzner_worker.pub")
  })
...
resource "hcloud_server" "worker-node-1" {
  ...
  user_data = templatefile("${path.module}/cloud-init-worker.yaml", {
    hello = var.hello,
    master_public_key = file("${path.module}/.mysecrets/id_rsa.hetzner.pub"),
    worker_private_key = file("${path.module}/.mysecrets/id_rsa.hetzner_worker")
  })
...
output xxx {
  value = hcloud_server.worker-node-1.user_data
}

output yyy {
  value = templatefile("${path.module}/cloud-init-worker.yaml", {
    master_public_key = file("${path.module}/.mysecrets/id_rsa.hetzner.pub"),
    worker_private_key = file("${path.module}/.mysecrets/id_rsa.hetzner_worker")
  })
}